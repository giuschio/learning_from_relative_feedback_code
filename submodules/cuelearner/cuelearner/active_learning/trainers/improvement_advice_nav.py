...
class AdviceWrapper:
    ...
    def select_action_batch(self, state):
        action = self.base_policy.select_action_batch(state)
        advised_action = self.advice_policy.select_action(state[0], action[0])
...
def eval_nav_policy(env, policy, n_steps):
    ...
    for _ in range(0, n_steps, n_envs):
        ...
        for index in range(n_envs):
            if done[index]:
                success = float(env.env.termination_manager.get_term('goal_reached')[index])
                collision = float(env.env.termination_manager.get_term('base_contact')[index])
    ...
    res = dict(
        ...
        val_episodes=len(episode_success)
    )
    return res
...
class Trainer:
    ...
    def fit(self, policy, advice_cloner, train_env, val_env, expert):
        ...
        success_train_p = []
        collision_train_p = []
        collisions_cumulative = 0
        success_cumulative = 0
        ...
        for step in range(1, self.max_steps, num_envs):
            ...
            # if adv_step > 2000:
            #     scheduler_adv_training.update_every = 50
            ...
            if scheduler_eval(adv_step):
                ...
                # policy_wrapper = AdviceWrapper(policy, advice_cloner)
                # val_res = eval_nav_policy(train_env, policy_wrapper, self.eval_steps)
            ...
            for index in range(num_envs):
                self.__log("episodes", num_episodes, step)
                if done[index]:
                    success = float(train_env.env.termination_manager.get_term('goal_reached')[index])
                    success_train_p.append(success)
                    success_cumulative += success
                    self.__log("train_successes_cumulative", success_cumulative, step)

                    collision = float(train_env.env.termination_manager.get_term('base_contact')[index])
                    collision_train_p.append(collision)
                    collisions_cumulative += collision
                    self.__log("train_collisions_cumulative", collisions_cumulative, step)
                    num_episodes += 1
