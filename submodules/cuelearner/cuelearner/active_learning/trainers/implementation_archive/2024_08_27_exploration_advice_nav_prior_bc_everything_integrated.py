...
class Trainer:
    ...
    def fit(self, policy, behavior_cloner, train_env, val_env, expert):
        ...
        for step in range(1, self.advice_scheduler.budget, num_envs):
            optimal_action = expert.select_action(train_env.get_navigation_observation())
            next_state, reward, done, terminated, info = train_env.step(optimal_action)
        ...
        print(f"training BC policy on {step} datapoints, roughly {int(step/300)} trajectories")
        ...
        use_advice = [True, True, True, True, True, True, False, False, False, False, False, False, ]
        ...
        for step in range(1, self.max_steps, num_envs):
            ...
            if step < self.start_steps:
                ...
                for index in range(num_envs):
                    if use_advice[index]:
                        action[index] = behavior_cloner.select_action(state[index])
