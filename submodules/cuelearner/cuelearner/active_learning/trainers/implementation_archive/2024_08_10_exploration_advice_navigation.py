...
class Trainer:
    ...
    def fit(self, policy, advice_cloner, train_env, val_env, expert):
        ...
        lookahead = 30
        transitions = deque(maxlen=lookahead)
        ...
        for step in range(1, self.max_steps, num_envs):
            ...
            if step < self.start_steps:
                ...
                #     # and (expected_reward < self.advice_q_threshold)
                #     and (step < self.advice_usage_limit)
                # )
                if use_advice:
                    timer.start("action_improvement")
                    # improve action
                    action = expert.select_action(train_env.get_navigation_observation())
                    # action = advice_cloner.select_action_batch(state=state, action=action)
            ...
            transitions.append(dict(state=state,
                                    action=action,
                                    next_state=next_state,
                                    reward=reward,
                                    done=done))

            # If we have enough transitions, add n-step transition to the replay buffer
            if len(transitions) == lookahead:
                gamma = policy.discount
                reward_10 = sum([(gamma ** t) * transitions[-t - 1]['reward'] for t in range(lookahead)])

                next_state_10 = transitions[-1]['next_state']
                done_10 = transitions[-1]['done']

                # obs_0, action_0, _, _, _ = transitions[0]

                replay_buffer.add_batch(
                    state=transitions[0]['state'],
                    action=transitions[0]['action'],
                    next_state=transitions[0]['next_state'],
                    reward=transitions[0]['reward'],
                    done=transitions[0]['done'],
                    next_state_10=next_state_10,
                    reward_10=reward_10,
                    done_10=done_10,
                    lookahead=np.array([float(len(transitions))])
                )

            # --- QUERY EXPERT FOR ADVICE
            # if use_advice:
            if False:
                if self.advice_scheduler(step):
                    timer.start("advice_query")

                    # improved_action = expert.get_advice(
                    #     state,
                    #     old_info,
                    #     action,
                    #     advice_step_size=advice_cloner.advice_step_size,
                    # )

                    # improvement_direction = get_user_choice()
                    # if improvement_direction is not None:
                    #     improved_action = rotate_vector(
                    #         action, 5.0 * improvement_direction
                    #     )
                    #     advice_buffer.add(
                    #         state=state,
                    #         action=action,
                    ...
                    #     )
                    #     # increase advice step counter
                    #     adv_step += 1
                    #     print("advice step: ", adv_step)
                    improved_action = expert.get_advice_batch(
                        state,
                        old_info,
                        action,
                        advice_step_size=advice_cloner.advice_step_size,
                    ...
                    timer.stop("advice_query")
            # END QUERY EXPERT FOR ADVICE
            ...
            if done[0]:
                print(f"reward{reward}", f"time {timer.get('total')}")
                # Reset environment
                # state, info = train_env.reset()
                # done = False
                # change advice allocation at the episode level instead of step level
                use_advice = (
                    (self.__rng.uniform(0, 1) < self.advice_probability)
                    and (step < self.advice_usage_limit))
                print(f"Switched to using advice: {use_advice}")

            if done[0]:
                while len(transitions) > 0:
                    gamma = policy.discount
                    reward_10 = sum([(gamma ** t) * transitions[-t - 1]['reward'] for t in range(len(transitions))])
