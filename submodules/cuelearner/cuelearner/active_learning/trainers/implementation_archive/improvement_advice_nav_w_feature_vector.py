...
class Trainer:
    ...
    def fit(self, policy, advice_cloner, train_env, val_env, expert):
        ...
        for step in range(1, self.max_steps, num_envs):
            ...
            feature_vector = policy.get_feature_vector(state, action)
            timer.start("action_improvement")

            advised_action = advice_cloner.select_action(np.concatenate((state[0], feature_vector[0])), action[0])
            ...
            if self.advice_scheduler(step):
                if expert == 'human':
                    ...
                    if human_feedback is not None:
                        ...
                        advice_buffer.add(
                            state=np.concatenate((state[0], feature_vector[0])),
                            action=action[0],
