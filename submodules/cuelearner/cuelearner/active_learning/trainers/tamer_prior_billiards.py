...
from cuelearner.common.players import compute_advice
...
class Trainer:
    ...
    def fit(self, tamer_policy, train_env, val_env, expert):
        # save network hyperparameters
        # dump_hparams(policy, self.log_dir.get_path("ckpt_policy/hparams.yaml"))
        dump_hparams(tamer_policy, self.log_dir.get_path("ckpt_advice/hparams.yaml"))
        ...
        for step in range(1, self.max_steps):
            action = tamer_policy.select_action(state)
            ...
            # ---- ADVICE END
            if reward == 0 and angle(action, optimal_action) < 5.0:
                reward = 1.0
            ...
            state = next_state
            # manual scheduling of the advice
            if step > 1000: scheduler_adv_training.update_every = 100

            if scheduler_adv_training(step):
                tamer_policy.set_train()
                res = tamer_policy.fit(advice_buffer, self.adv_grad_steps)
                self.__log_dict(res, step)
                self.__log("step", step, step)
                tamer_policy.set_eval()
                self.__log("train_reward", train_reward_p, step)
                train_reward_p = []
                print(f"Training dagger policy at step {step}")

            if scheduler_eval(step):
                with torch.no_grad():
                    final_rewards = eval_policy(
                        tamer_policy, dcp(val_env), self.eval_steps
                    )
                self.__log("val_reward", final_rewards, step)
                self.__log("step", step, step)
                self.__log_dict(timer.get_all(), step)
                self.__advice_ckpt_manager.update(
                    state_dict=tamer_policy.get_state_dict(),
                    epoch=step,
