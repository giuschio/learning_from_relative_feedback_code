...
class Trainer:
    ...
    def fit(self, tamer_agent, train_env, val_env, expert):
        # save network hyperparameters
        # dump_hparams(policy, self.log_dir.get_path("ckpt_policy/hparams.yaml"))
        dump_hparams(tamer_agent, self.log_dir.get_path("ckpt_advice/hparams.yaml"))
        ...
        for step in range(1, self.max_steps, num_envs):
            ...
            action = tamer_agent.select_action_batch(state)
            timer.stop("action_choice")
            old_info = info

            timer.start("a_star")
            optimal_action = expert.select_action(train_env.get_navigation_observation())
            # try to use an easier-to-learn optimal action...
            # optimal_action = train_env.get_navigation_observation()['goal_position']
            # optimal_action /= np.linalg.norm(optimal_action)
            timer.stop("a_star")
            timer.start("simulator")
            next_state, _, done, terminated, info = train_env.step(action)
            timer.stop("simulator")

            # --- COMPUTE AND SAVE ADVICE
            for index in range(num_envs):
                action_error = angle(optimal_action, action[index])
                if action_error < 10:
                    # same error tolerance of the relative advice learner
                    reward = 1.0
                    adv_step += 1
                else:
                    reward = 0.
